...
const dictionaries = {
  ko: {
    ...
    'failure.kind.installer-busy': '설치 충돌',
    'failure.kind.install-tech': '재설치 필요',
    ...
    'failure.kind.uninstall': '삭제 실패',
    'failure.kind.reinstall': '재설치 실패',
    ...
    'failure.advice.installer-busy': '다른 설치(Windows Installer)가 진행 중이어서 실패했습니다(오류 1618). Windows 업데이트나 직전 패키지의 설치 마무리가 끝나기를 기다린 뒤 이 항목만 다시 시도하세요.',
    'failure.advice.install-tech': '새 버전의 설치 방식이 현재 설치된 버전과 달라 제자리 업그레이드가 불가능합니다(예: Edge, Oh My Posh). 재시도해도 같은 결과이니, 해당 앱을 제거한 뒤 최신 버전을 새로 설치하거나 앱 자체 업데이트 기능을 사용하세요.',
    ...
    'failure.advice.uninstall': '관련 앱을 종료한 뒤 다시 시도하세요. 계속 실패하면 관리자 권한으로 재시작하거나 무인 설치 옵션을 끄고 삭제하세요.',
    'failure.advice.reinstall-uninstall': '제거 단계에서 실패해 설치는 시도하지 않았습니다(기존 앱은 그대로 남아 있음). 관련 앱을 종료한 뒤 다시 시도하고, 계속 실패하면 관리자 권한으로 재시작하세요.',
    'failure.advice.reinstall-install': '제거 후 설치 단계에서 실패해 앱이 제거된 상태일 수 있습니다. 재설치를 다시 실행해 최신 버전 설치를 완료하세요.',
    'failure.advice.generic': '로그의 마지막 오류를 확인하고 같은 항목만 다시 시도하세요.',
    'failure.summary.installer': '설치 오류 {count}',
    'failure.summary.installer-busy': '설치 충돌 {count}',
    'failure.summary.install-tech': '재설치 필요 {count}',
    ...
    'failure.summary.uninstall': '삭제 {count}',
    'failure.summary.reinstall': '재설치 실패 {count}',
    'failure.summary.generic': '기타 {count}',
    'language.title': '표시 언어',
    'language.system': '시스템 언어',
    'language.ko': '한국어',
    'language.en': 'English',
    'tooltips.silent': '설치 또는 삭제 프로그램이 지원하면 확인 창 없이 조용히 처리합니다.',
    ...
    'tooltips.reinstall': '설치 기술이 바뀌어 제자리 업그레이드가 불가능한 앱을 제거한 뒤 최신 버전을 새로 설치합니다.',
    ...
    'progress.current': '{position}/{total} · {name}',
    'progress.preparing': '다음 작업을 준비하는 중',
    'progress.completed': '전체 {total}개 중 {completed}개 처리 완료 (성공·실패 포함)',
    'progress.queue': '전체 패키지 처리 진행률',
    'progress.ready': '업데이트할 패키지를 선택하세요',
    'progress.action.upgrade': '업데이트 중',
    'progress.action.uninstall': '삭제 중',
    'progress.action.reinstall': '재설치 중',
    'progress.count': '{completed}/{total}개 처리',
    'progress.uninstalling': '선택한 앱을 순차 삭제하는 중',
    'progress.reinstalling': '대상 앱을 제거 후 최신 버전으로 재설치하는 중',
    ...
    'logs.uninstallStart': '선택 삭제 시작: {count}개',
    'logs.uninstallComplete': '선택 삭제 완료',
    'logs.reinstallStart': '재설치 시작: {count}개',
    'logs.reinstallComplete': '재설치 완료',
    'logs.reinstallExcluded': '자체 업데이트 방식 앱은 재설치 대상에서 제외: {name}',
    ...
    'actions.retryFailedUninstall': '삭제 재시도',
    'actions.reinstall': '재설치',
    'actions.confirmReinstall': '{count}개 재설치',
    'actions.uninstallSelected': '선택 삭제',
    'actions.confirmUninstall': '{count}개 삭제',
    'actions.keep': '유지',
    ...
    'table.source': '원본',
    'table.status': '상태',
    'uninstall.kicker': '되돌릴 수 없는 작업',
    'uninstall.title': '선택한 앱 {count}개를 삭제할까요?',
    'uninstall.description': 'winget이 각 앱의 제거 프로그램을 실행합니다. 완료된 앱은 이 목록에서 사라집니다.',
    'uninstall.warning': '앱의 로컬 데이터와 설정도 함께 지워질 수 있습니다. 필요한 작업을 저장하고 관련 앱을 먼저 종료하세요.',
    'uninstall.more': '그 외 {count}개 앱',
    'reinstall.kicker': '제거 후 새로 설치',
    'reinstall.title': '앱 {count}개를 재설치할까요?',
    'reinstall.description': 'winget이 각 앱을 제거한 뒤 최신 버전을 새로 설치합니다. 설치 기술이 바뀌어 제자리 업그레이드가 불가능한 앱을 최신으로 올리는 방법입니다.',
    'reinstall.warning': '제거는 성공했지만 설치가 실패하면 앱이 제거된 상태로 남을 수 있습니다. 관련 앱을 먼저 종료하고 진행하세요.',
  ...
  en: {
    ...
    'failure.kind.installer-busy': 'Install conflict',
    'failure.kind.install-tech': 'Reinstall needed',
    ...
    'failure.kind.uninstall': 'Uninstall failed',
    'failure.kind.reinstall': 'Reinstall failed',
    ...
    'failure.advice.installer-busy': 'Another installation (Windows Installer) was in progress, so this one failed (error 1618). Wait for Windows Update or the previous package to finish installing, then retry only this item.',
    'failure.advice.install-tech': 'The new version uses a different install technology than the installed one (e.g. Edge, Oh My Posh), so an in-place upgrade is not possible. Retrying will not help — uninstall the app and install the latest version, or use the app’s own updater.',
    ...
    'failure.advice.uninstall': 'Close the related app and retry. If it still fails, restart as administrator or turn off silent install before uninstalling.',
    'failure.advice.reinstall-uninstall': 'The uninstall step failed, so the install was not attempted (the existing app is untouched). Close the related app and retry; if it keeps failing, restart as administrator.',
    'failure.advice.reinstall-install': 'The install step failed after the uninstall, so the app may currently be uninstalled. Run the reinstall again to finish installing the latest version.',
    ...
    'failure.summary.installer-busy': 'install conflict {count}',
    'failure.summary.install-tech': 'reinstall {count}',
    ...
    'failure.summary.uninstall': 'uninstall {count}',
    'failure.summary.reinstall': 'reinstall failed {count}',
    'failure.summary.generic': 'other {count}',
    'language.title': 'Display Language',
    'language.system': 'System language',
    'language.ko': '한국어',
    'language.en': 'English',
    'tooltips.silent': 'Installs or uninstalls quietly without prompts when the package supports it.',
    ...
    'tooltips.reinstall': 'Uninstalls apps whose new version uses a different install technology, then installs the latest version fresh.',
    ...
    'progress.current': '{position}/{total} · {name}',
    'progress.preparing': 'Preparing the next operation',
    'progress.completed': '{completed} of {total} packages processed (including success and failure)',
    'progress.queue': 'Overall package processing progress',
    'progress.ready': 'Select packages to update',
    'progress.action.upgrade': 'Updating',
    'progress.action.uninstall': 'Uninstalling',
    'progress.action.reinstall': 'Reinstalling',
    'progress.count': '{completed}/{total} processed',
    'progress.uninstalling': 'Uninstalling selected apps in order',
    'progress.reinstalling': 'Reinstalling target apps (uninstall, then latest install)',
    ...
    'logs.uninstallStart': 'Selected uninstall started: {count}',
    'logs.uninstallComplete': 'Selected uninstall complete',
    'logs.reinstallStart': 'Reinstall started: {count}',
    'logs.reinstallComplete': 'Reinstall complete',
    'logs.reinstallExcluded': 'Excluded self-updating app from reinstall: {name}',
    ...
    'actions.retryFailedUninstall': 'Retry Uninstall',
    'actions.reinstall': 'Reinstall',
    'actions.confirmReinstall': 'Reinstall {count}',
    'actions.uninstallSelected': 'Uninstall Selected',
    'actions.confirmUninstall': 'Uninstall {count}',
    'actions.keep': 'Keep Apps',
    ...
    'table.source': 'Source',
    'table.status': 'Status',
    'uninstall.kicker': 'Destructive action',
    'uninstall.title': 'Uninstall {count} selected apps?',
    'uninstall.description': 'winget will run each app’s uninstaller. Completed apps will disappear from this list.',
    'uninstall.warning': 'Local app data and settings may also be removed. Save your work and close the related apps first.',
    'uninstall.more': '{count} more apps',
    'reinstall.kicker': 'Uninstall, then install fresh',
    'reinstall.title': 'Reinstall {count} apps?',
    'reinstall.description': 'winget will uninstall each app and then install the latest version fresh. This is how apps whose install technology changed get updated.',
    'reinstall.warning': 'If the uninstall succeeds but the install fails, the app may be left uninstalled. Close the related apps before proceeding.',
