...
개발자가 경고를 줄이려면 Windows 코드 서명 인증서로 릴리즈 exe를 서명해야 합니다. EV 코드 서명 인증서는 평판 확보가 가장 빠르고, OV/일반 코드 서명 인증서는 다운로드와 실행 이력이 쌓이며 점진적으로 경고가 줄어들 수 있습니다. 서명 인증서가 없는 초기 릴리즈에서는 사용자가 SmartScreen의 `추가 정보`에서 실행을 선택해야 할 수 있습니다.
